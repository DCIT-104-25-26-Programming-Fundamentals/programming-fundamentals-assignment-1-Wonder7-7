...
def add_task(tasks):
    """Prompt for a task description and add it to `tasks`."""
...
def view_tasks(tasks):
    """Print all tasks, numbered from 1."""
    if not tasks:
...
def delete_task(tasks):
    """Show the task list and remove the one the user picks."""
    if not tasks:
        print("There are no tasks to delete.")
        return

    view_tasks(tasks)
    raw = input("Enter task number to delete: ")
